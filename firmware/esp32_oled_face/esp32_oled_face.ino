/*
  ==============================================================================
  OpenPets / Tink — ESP32 Dual-Core Pet Body Controller (esp32_oled_face)
  ==============================================================================
  ...
  - CORE 1 (APP_CPU): Dedicated OLED Display & Serial Frame Receiver
    * 128x64 OLED display over I2C at 800 kHz.
...
void updateServoMotion() {
  ...
  if (now - lastServoStepTime >= servoStepIntervalMs) {
    ...
    ledcWrite(SERVO_CHANNEL, angleToDuty(servoCurrentAngle));
  }
}
...
void handleCommand(const String &cmd) {
  if (cmd == "SERVO:FAST") {
    ...
    ledcWrite(SERVO_CHANNEL, angleToDuty(SERVO_CENTER_ANGLE)); // Park at 35°
...
void setup() {
  ...
  // Setup Servo on GPIO 13 via LEDC PWM
  ledcSetup(SERVO_CHANNEL, SERVO_FREQ, SERVO_RES);
  ledcAttachPin(SERVO_PIN, SERVO_CHANNEL);
  ledcWrite(SERVO_CHANNEL, angleToDuty(SERVO_CENTER_ANGLE));
